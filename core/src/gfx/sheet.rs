//! Tile/sprite sheet compositing shared by the offline `gfxview` PNG export and
//! the interactive frontend viewer.
//!
//! A decoded [`GfxCache`] holds `count` elements, each a `width × height` grid of
//! palette indices. [`render_sheet`] lays those elements out in a column-major
//! grid, colors each index through a palette (or a grayscale ramp for machines
//! with no color PROM), and nearest-neighbor upscales by an integer factor. The
//! result is a plain RGB24 buffer — encoding it (PNG, SDL texture, …) is the
//! caller's job, keeping this module free of any I/O or external dependencies.
