...
- `host_keys.rs` - Rebindable *emulator* hotkeys (as opposed to game input): `HostAction`/`HostBindings` plus the `DEFAULTS` table. `DEFAULTS` is the authority for which key does what — `emulator.rs` dispatches on `HostAction`, so a comment there naming a concrete key is only a hint and goes stale silently. The function-key layout follows MAME's, deliberately, since that is what an arcade emulator user already has in their fingers; `DEFAULTS` documents each deviation and why. `HostBindings` maps one `HostChord` per action: a scancode plus shift, matched on the *exact* modifier state so bare `F5` and `Shift+F5` are separate bindings. Ctrl and Alt are not modelled, so the console toggle (Ctrl+`` ` ``) stays hardcoded in `emulator.rs` and is not rebindable. Also reports hotkeys that shadow a machine's own controls (only bare chords can, since game input ignores modifiers)
- `movie.rs` - Input-movie capture (`--record PATH`, or Shift+F12 mid-session): `Recording`, a tee over `InputConfigurable` that forwards each call to the machine and appends it to a `phosphor-harness` `MovieRecorder`. Deliberately a wrapper rather than a hook inside `input.rs`: `dispatch` and `resync` are unchanged and unaware, so an input path cannot be recorded incompletely without also bypassing the trait. Arming resets the machine, because a movie carries no save state and replays only from power-on. `--record` arms through the same deferred path as the hotkey before the first frame and writes on quit, so both recordings start from a rebuilt-from-ROM machine and neither depends on catching a keypress. Replay is `--movie`, `disasm replay` / `disasm movie check`
...
- `profile.rs` - Profiler side panel (Shift+F11): per-frame input/emulation/audio/render/idle timings, machine-supplied `ProfileSpan` sub-spans, and Chrome Trace Event export
- `settings_ui.rs` - egui settings panels: the input rebinding panel (Shift+F1) with click-to-capture for both machine controls and host hotkeys, the DIP switch panel (`` ` ``), and the key legend (`?`). Bindings persist per machine in `state.toml`
...
- `logging.rs` - Installs the `log` subscriber (`env_logger`) that every diagnostic in this crate goes through, and documents which level means what. Formatted like cargo rather than like a log file: an `info!` prints bare, because that level is reserved for confirming something the user just did ("Screenshot saved: …"), while `warn!`/`error!` take a prefix and `debug!` keeps its target so `RUST_LOG=phosphor_frontend::video=debug` can select it. Fatal startup errors (unknown machine, missing ROM path) deliberately stay on `eprintln!` in `main.rs`: they are usage output, and `RUST_LOG` must not be able to silence them
