...
fn test_render_frame_uses_palette() {
    ...
    sys.bus_write(BusMaster::Cpu(1), 0x0000, 0x20); // sound CPU BRA *
    sys.bus_write(BusMaster::Cpu(1), 0x0001, 0xFE);

    // Set palette entry 5 to a known BBGGGRRR color: R=7, G=0, B=3 => 0b11_000_111 = 0xC7
    sys.bus_write(BusMaster::Cpu(0), 0xC005, 0xC7);
...
fn test_render_frame_two_pixels_per_byte() {
    ...
    sys.bus_write(BusMaster::Cpu(1), 0x0000, 0x20);
    sys.bus_write(BusMaster::Cpu(1), 0x0001, 0xFE);

    // Palette entry 3 (BBGGGRRR): R=0, G=7, B=0 => 0b00_111_000 = 0x38
    // Palette entry 9 (BBGGGRRR): R=4, G=4, B=2 => 0b10_100_100 = 0xA4
    sys.bus_write(BusMaster::Cpu(0), 0xC003, 0x38);
    sys.bus_write(BusMaster::Cpu(0), 0xC009, 0xA4);
...
fn test_set_input_active_high() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0x00); // DDRA = 0 (all input)
    sys.bus_write(BusMaster::Cpu(0), 0xC805, 0x04); // CRA: bit 2 = 1 (data select)

    // All buttons start released (0x00), Port A should read 0x00
    let val = sys.bus_read(BusMaster::Cpu(0), 0xC804);
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC807, 0x3C); // CRB: bits 5:4:3 = 111 → CB2 output high
...
fn test_set_input_multiple_buttons() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0x00); // DDRA = 0
    sys.bus_write(BusMaster::Cpu(0), 0xC805, 0x04); // CRA data select
    sys.bus_write(BusMaster::Cpu(0), 0xC807, 0x3C); // CRB: CB2 output high (P1)
...
fn test_set_input_coin() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC80C, 0x00); // DDRA = 0
    sys.bus_write(BusMaster::Cpu(0), 0xC80D, 0x04); // CRA data select
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0xC80C);
...
fn test_input_muxing() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC807, 0x3C); // CRB: CB2 output high
    let val = sys.bus_read(BusMaster::Cpu(0), 0xC804);
...
fn test_video_ram_via_bus() {
    let mut sys = JoustSystem::new();
    sys.bus_write(BusMaster::Cpu(0), 0x1234, 0xCD);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x1234), 0xCD);
...
fn test_palette_ram_read_write() {
    let mut sys = JoustSystem::new();
    sys.bus_write(BusMaster::Cpu(0), 0xC000, 0xAA);
    sys.bus_write(BusMaster::Cpu(0), 0xC00F, 0xBB);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC000), 0xAA);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC00F), 0xBB);
...
fn test_cmos_ram_read_write() {
    let mut sys = JoustSystem::new();
    sys.bus_write(BusMaster::Cpu(0), 0xCC00, 0x42);
    sys.bus_write(BusMaster::Cpu(0), 0xCFFF, 0x99);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xCC00), 0xF2);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xCFFF), 0xF9);
}
...
fn test_rom_write_protection() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xD000, 0x55);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xD000), 0xAA);
}
...
fn test_unmapped_returns_ff() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC010), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC100), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC7FF), 0xFF);
}
...
fn test_widget_pia_accessible() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0xFF); // Set all as outputs
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC804), 0xFF);
}
...
fn test_rom_pia_accessible() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC80C, 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC80C), 0xFF);
}
...
fn test_pia_addresses_independent() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0xAA); // Widget PIA DDRA
    sys.bus_write(BusMaster::Cpu(0), 0xC80C, 0x55); // ROM PIA DDRA
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC804), 0xAA);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC80C), 0x55);
}
...
fn test_blitter_registers_accessible() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA01, 0x42); // solid_color
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 1 ^ 4); // width (SC1 XOR 4 → effective 1)
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 1 ^ 4); // height (SC1 XOR 4 → effective 1)

    // Reads return 0 (write-only registers)
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xCA00), 0);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xCA01), 0);

    // Writing $CA00 triggers the blit — verify CPU gets halted
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x00); // control: fast copy, triggers blit
    assert!(sys.board.is_halted_for(BusMaster::Cpu(0)));
}
...
fn test_blitter_halts_cpu() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA01, 0x42); // solid_color
    sys.bus_write(BusMaster::Cpu(0), 0xCA04, 0x00); // dst_hi
    sys.bus_write(BusMaster::Cpu(0), 0xCA05, 0x00); // dst_lo
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 8 ^ 4); // width = 8, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 8 ^ 4); // height = 8, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x10); // control: SOLID (bit 4), fast, triggers blit
...
fn test_blitter_writes_to_video_ram() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA02, 0x10); // src_hi = 0x10
    sys.bus_write(BusMaster::Cpu(0), 0xCA03, 0x00); // src_lo = 0x00
    sys.bus_write(BusMaster::Cpu(0), 0xCA04, 0x20); // dst_hi = 0x20
    sys.bus_write(BusMaster::Cpu(0), 0xCA05, 0x00); // dst_lo = 0x00
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 2 ^ 4); // width = 2 bytes, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 1 ^ 4); // height = 1 row, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x00); // control: fast linear copy, triggers blit
...
fn test_load_program_rom_slice() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xD000), 0x12);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xD001), 0x34);
}
...
fn test_load_rom_set_by_name_fallback() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xD000), 0x11);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xE000), 0x22);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xF000), 0x33);
}
...
fn test_rom_bank_select() {
    let mut sys = JoustSystem::new();
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x03);
    assert_eq!(sys.board.rom_bank(), 0x03);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xC900), 0x03);
}
...
fn test_bank_switch_disabled_reads_video_ram() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x1000), 0xBB);
}
...
fn test_bank_switch_enabled_reads_banked_rom() {
    ...
    // Enable ROM bank
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x1000), 0xAA);
}
...
fn test_bank_switch_writes_always_to_video_ram() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    sys.bus_write(BusMaster::Cpu(0), 0x2000, 0xCC);
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x00);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x2000), 0xCC);

    // Re-enable bank: ROM should still be intact (write went to video RAM, not ROM)
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x2000), 0xAA);
}
...
fn test_upper_video_ram_unaffected_by_bank() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x9000), 0x55);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0xBFFF), 0x66);
}
...
fn test_bank_switch_boundary_addresses() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x8FFF), 0xDD);
    // 0x9000 should return video RAM data (outside banked range)
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x9000), 0xFF);
}
...
fn test_bank_switch_address_zero() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x00);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x0000), 0x99);
}
...
fn test_bank_switch_toggle() {
    ...
    // Off -> On -> Off -> On
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4000), 0x22);
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4000), 0x11);
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x00);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4000), 0x22);
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4000), 0x11);
...
fn test_blitter_reads_banked_rom() {
    ...
    // Write data registers first, then $CA00 last to trigger.
    sys.bus_write(BusMaster::Cpu(0), 0xCA02, 0x10); // src_hi
    sys.bus_write(BusMaster::Cpu(0), 0xCA03, 0x00); // src_lo
    sys.bus_write(BusMaster::Cpu(0), 0xCA04, 0x90); // dst_hi
    sys.bus_write(BusMaster::Cpu(0), 0xCA05, 0x00); // dst_lo
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 2 ^ 4); // width = 2 bytes, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 1 ^ 4); // height = 1 row, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x00); // control: fast linear copy, triggers blit
...
fn test_blitter_dest_read_bypasses_rom_banking() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA02, 0x90); // src_hi
    sys.bus_write(BusMaster::Cpu(0), 0xCA03, 0x00); // src_lo
    sys.bus_write(BusMaster::Cpu(0), 0xCA04, 0x20); // dst_hi
    sys.bus_write(BusMaster::Cpu(0), 0xCA05, 0x00); // dst_lo
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 1 ^ 4); // width = 1, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 1 ^ 4); // height = 1, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x08); // control: FOREGROUND_ONLY, triggers blit
...
fn test_reset_clears_bank_preserves_rom() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC900, 0x01);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x0000), 0xAA);
}
...
fn test_load_banked_rom_all_chips() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x0000), 0x01); // chip 1
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x1000), 0x02); // chip 2
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x2000), 0x03); // chip 3
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x3000), 0x04); // chip 4
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4000), 0x05); // chip 5
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x5000), 0x06); // chip 6
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x6000), 0x07); // chip 7
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x7000), 0x08); // chip 8
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x8000), 0x09); // chip 9
}
...
fn test_video_counter_read() {
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0xCB00);
    assert_eq!(val, 0x00);
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0xCB42); // Any address in 0xCB00-0xCBFF works
    assert_eq!(val, 0x04);
}
...
fn test_watchdog_reset_on_write() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCBFF, 0x39);
...
fn test_watchdog_ignores_non_0x39() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCBFF, 0x00);
    assert_eq!(sys.board.watchdog_counter, before);
    sys.bus_write(BusMaster::Cpu(0), 0xCBFF, 0xFF);
    assert_eq!(sys.board.watchdog_counter, before);
}
...
fn test_sound_cpu_executes_independently() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0010), 0x42);
...
fn test_sound_ram_isolated_from_main_cpu() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x0010, 0xAA);
    // Write to address 0x0010 via sound CPU (hits sound RAM)
    sys.bus_write(BusMaster::Cpu(1), 0x0010, 0xBB);

    // They should be in separate memory spaces
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x0010), 0xAA);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0010), 0xBB);
}
...
fn test_sound_rom_readable() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xF000), 0xDE);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xF001), 0xAD);
}
...
fn test_sound_rom_mirrored_at_0xb000() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xB000), 0xDE);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xB001), 0xAD);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xC000), 0xDE);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xD000), 0xDE);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xE000), 0xDE);
}
...
fn test_sound_rom_write_protection() {
    ...
    sys.bus_write(BusMaster::Cpu(1), 0xF000, 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xF000), 0x42);
}

#[test]
fn test_sound_unmapped_returns_ff() {
    let mut sys = JoustSystem::new();
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0100), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0500), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x1000), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0xAFFF), 0xFF);
}
...
fn test_sound_pia_accessible() {
    ...
    sys.bus_write(BusMaster::Cpu(1), 0x0400, 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0400), 0xFF);
}
...
fn test_sound_command_propagation() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC80E, 0xFF); // ROM PIA DDRB
    sys.bus_write(BusMaster::Cpu(0), 0xC80F, 0x04); // CRB: bit 2 = 1 (data select)

    // Configure sound PIA Port B as all input (default) with CRB data select
    sys.bus_write(BusMaster::Cpu(1), 0x0403, 0x04); // Sound PIA CRB: data select

    // Write a sound command to ROM PIA Port B
    sys.bus_write(BusMaster::Cpu(0), 0xC80E, 0x42);

    // Tick once to propagate
    sys.tick();

    // Sound PIA Port B should have the command with high bits pulled up (| 0xC0)
    let command = sys.bus_read(BusMaster::Cpu(1), 0x0402);
...
fn test_sound_cpu_not_halted_during_blit() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA01, 0x00);
    sys.bus_write(BusMaster::Cpu(0), 0xCA04, 0x90);
    sys.bus_write(BusMaster::Cpu(0), 0xCA05, 0x00);
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 8 ^ 4);
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 8 ^ 4);
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x10); // SOLID blit
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(1), 0x0050), 0x42);
}
...
fn test_scanline_va11_signal() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC80E, 0x00); // DDRB = 0
    sys.bus_write(BusMaster::Cpu(0), 0xC80F, 0x07); // CRB: enable CB1 IRQ, rising edge, data select
    ...
    let crb = sys.bus_read(BusMaster::Cpu(0), 0xC80F);
    assert_ne!(
        crb & 0x80,
        0,
...
fn test_scanline_count240_signal() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC80C, 0x00); // DDRA = 0
    sys.bus_write(BusMaster::Cpu(0), 0xC80D, 0x07); // CRA: enable CA1 IRQ, rising edge, data select
    ...
    let cra = sys.bus_read(BusMaster::Cpu(0), 0xC80D);
    assert_ne!(
        cra & 0x80,
        0,
...
fn test_irq_routing_rom_pia_only() {
    ...
    let state = Bus::check_interrupts(&mut sys.board, BusMaster::Cpu(0));
    assert!(!state.irq);
    assert!(
        !state.firq,
