...
fn test_fire_stick_port_b_wiring() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC806, 0x00); // DDRB = 0
    sys.bus_write(BusMaster::Cpu(0), 0xC807, 0x04); // CRB data select
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0xC806);
...
fn test_start_buttons_wiring() {
    ...
    // Configure Widget PIA Port A as all input, data select
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0x00);
    sys.bus_write(BusMaster::Cpu(0), 0xC805, 0x04);
...
fn test_blitter_writes_to_video_ram() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xCA06, 2 ^ 4); // width = 2, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA07, 1 ^ 4); // height = 1, XOR 4 for SC1
    sys.bus_write(BusMaster::Cpu(0), 0xCA00, 0x00); // control: triggers blit
...
fn test_reset_clears_input_state() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0xC804, 0x00);
    sys.bus_write(BusMaster::Cpu(0), 0xC805, 0x04);
    sys.bus_write(BusMaster::Cpu(0), 0xC806, 0x00);
    sys.bus_write(BusMaster::Cpu(0), 0xC807, 0x04);
    ...
    let port_a = sys.bus_read(BusMaster::Cpu(0), 0xC804);
    let port_b = sys.bus_read(BusMaster::Cpu(0), 0xC806);
