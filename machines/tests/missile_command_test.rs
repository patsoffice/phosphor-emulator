...
fn test_ram_read_write() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x0000, 0xAA);
    sys.bus_write(BusMaster::Cpu(0), 0x3FFF, 0xBB);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x0000), 0xAA);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x3FFF), 0xBB);
}
...
fn test_ram_via_helper() {
    ...
    assert_eq!(sys.board.read_ram(0x1234), 0xCD);
}
...
fn test_rom_not_writable() {
    ...
    sys.board.write_ram(0x0000, 0x11); // RAM is writable
    // ROM at 0x5000+ should not be writable through the bus
    sys.bus_write(BusMaster::Cpu(0), 0x5000, 0xFF);
    // ROM should still be 0 (default)
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x5000), 0x00);
}
...
fn test_rom_mirror_vectors() {
    ...
    assert_eq!(
        sys.bus_read(BusMaster::Cpu(0), 0x7800),
        sys.bus_read(BusMaster::Cpu(0), 0xF800)
    );
    assert_eq!(
        sys.bus_read(BusMaster::Cpu(0), 0x7FFF),
        sys.bus_read(BusMaster::Cpu(0), 0xFFFF)
    );
}
...
fn test_unmapped_returns_ff() {
    ...
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4E00), 0xFF);
    assert_eq!(sys.bus_read(BusMaster::Cpu(0), 0x4F00), 0xFF);
}
...
fn test_pokey_accessible() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4008, 0x00);
    // Read ALLPOT (offset 0x08) — this is a read-only register
    let _val = sys.bus_read(BusMaster::Cpu(0), 0x4008);
    // Should not panic
}
...
fn test_pokey_mirror() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4010, 0x42);
    // Reading back from the mirrored offset should also work
    let _val = sys.bus_read(BusMaster::Cpu(0), 0x4410);
...
fn test_in0_default_all_released() {
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0x4800);
    assert_eq!(
        val, 0xFF,
...
fn test_in1_fire_buttons_default() {
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0x4900);
...
fn test_in1_fire_on_separate_register_from_in0() {
    ...
    let in0 = sys.bus_read(BusMaster::Cpu(0), 0x4800);
    ...
    let in1 = sys.bus_read(BusMaster::Cpu(0), 0x4900);
    assert_eq!(
        in1 & 0x07,
        0x03,
...
fn test_ctrld_set_reads_trackball() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4800, 0x01);
...
fn test_ctrld_trackball_values() {
    let mut sys = MissileCommandSystem::new();
    sys.bus_write(BusMaster::Cpu(0), 0x4800, 0x01); // CTRLD=1
...
fn test_ctrld_toggle_back_to_switches() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4800, 0x01);
    sys.bus_write(BusMaster::Cpu(0), 0x4800, 0x00);
...
fn test_dip_switches_readable() {
    ...
    let val = sys.bus_read(BusMaster::Cpu(0), 0x4A00);
...
fn test_color_ram_write_read() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E); // Entry 0: bits 3/2/1 = R/G/B inverted
    sys.bus_write(BusMaster::Cpu(0), 0x4B07, 0x02); // Entry 7
    assert_eq!(sys.board.read_palette(0), 0x0E);
    assert_eq!(sys.board.read_palette(7), 0x02);
}
...
fn test_color_ram_mirror() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B08, 0xAA);
...
fn test_watchdog_reset_write() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4C00, 0x00);
    // No panic = success
}
...
fn test_irq_acknowledge_clears_irq() {
    ...
    sys.step_cycle(); // clock was 0, frame_cycle=0, scanline=0

    // Check that IRQ is asserted
    let _in1 = sys.bus_read(BusMaster::Cpu(0), 0x4900); // IN1 — not the IRQ itself
    // The IRQ state is reported through check_interrupts, which we test indirectly

    // Write to 0x4D00 to acknowledge IRQ
    sys.bus_write(BusMaster::Cpu(0), 0x4D00, 0x00);
...
fn test_render_palette_colors() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E); // bits 3=1, 2=1, 1=1 → R=0, G=0, B=0
...
fn test_render_bit_planar_pixel_extraction() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E); // idx 0: black
    sys.bus_write(BusMaster::Cpu(0), 0x4B02, 0x0C); // idx 2: blue
    sys.bus_write(BusMaster::Cpu(0), 0x4B04, 0x06); // idx 4: red
    sys.bus_write(BusMaster::Cpu(0), 0x4B06, 0x04); // idx 6: magenta
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x0640, 0x21);
...
fn test_render_all_four_pixels_in_byte() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E);
    sys.bus_write(BusMaster::Cpu(0), 0x4B02, 0x0C);
    sys.bus_write(BusMaster::Cpu(0), 0x4B04, 0x06);
    sys.bus_write(BusMaster::Cpu(0), 0x4B06, 0x04);
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x0640, 0x35);
...
fn test_in0_address_mirror() {
    ...
    let val_base = sys.bus_read(BusMaster::Cpu(0), 0x4800);
    let val_mirror = sys.bus_read(BusMaster::Cpu(0), 0x48FF);
    assert_eq!(
        val_base, val_mirror,
...
fn test_in1_address_mirror() {
    ...
    let val_base = sys.bus_read(BusMaster::Cpu(0), 0x4900);
    let val_mirror = sys.bus_read(BusMaster::Cpu(0), 0x4955);
    assert_eq!(
        val_base, val_mirror,
...
fn test_irq_ack_address_mirror() {
    ...
    sys.step_cycle(); // trigger IRQ at scanline 0
    sys.bus_write(BusMaster::Cpu(0), 0x4DFF, 0x00); // mirror write
...
fn test_frame_timing() {
    ...
    assert_eq!(sys.board.clock(), 20480, "One frame should be 20480 cycles");
}
...
fn test_scanline_counter() {
    let mut sys = MissileCommandSystem::new();
    assert_eq!(
        sys.board.current_scanline(),
        0,
        "Scanline should start at 0"
    );
...
fn test_15bit_masking_ram_alias() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x0100, 0xAB);
    // Read from 0x8100 should alias to 0x0100 (0x8100 & 0x7FFF = 0x0100)
    assert_eq!(
        sys.bus_read(BusMaster::Cpu(0), 0x8100),
        0xAB,
...
fn test_15bit_masking_rom_vectors() {
    ...
    let via_direct = sys.bus_read(BusMaster::Cpu(0), 0x7FFC);
    let via_mask = sys.bus_read(BusMaster::Cpu(0), 0xFFFC);
    assert_eq!(
        via_direct, via_mask,
...
fn test_15bit_masking_write() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x8200, 0xCD);
    assert_eq!(
        sys.board.read_ram(0x0200),
        0xCD,
...
fn test_madsel_write_via_cpu() {
    ...
    sys.board.write_ram(0x0000, 0x78); // SEI
    sys.board.write_ram(0x0001, 0xA9); // LDA #imm
    sys.board.write_ram(0x0002, 0x00); // #$00
    sys.board.write_ram(0x0003, 0x8D); // STA abs
    sys.board.write_ram(0x0004, 0x00); // addr low
    sys.board.write_ram(0x0005, 0x4D); // addr high (0x4D00 → IRQ ack)
    sys.board.write_ram(0x0006, 0xA9); // LDA #imm
    sys.board.write_ram(0x0007, 0xC0); // #$C0
    sys.board.write_ram(0x0008, 0xA2); // LDX #imm
    sys.board.write_ram(0x0009, 0x00); // #$00
    sys.board.write_ram(0x000A, 0x81); // STA (zp,X) → opcode 0x81 triggers MADSEL
    sys.board.write_ram(0x000B, 0x10); // zp addr $10
    sys.board.write_ram(0x000C, 0x4C); // JMP abs
    sys.board.write_ram(0x000D, 0x0C); // addr low
    sys.board.write_ram(0x000E, 0x00); // addr high (JMP $000C)

    // Set up zero-page pointer at $10/$11 → pixel address $0800
    // VRAM byte address = $0800 >> 2 = $0200, pixel = $0800 & 3 = 0
    sys.board.write_ram(0x0010, 0x00); // pointer low byte
    sys.board.write_ram(0x0011, 0x08); // pointer high byte

    // Verify VRAM at 0x0200 is initially 0
    assert_eq!(
        sys.board.read_ram(0x0200),
        0x00,
        "VRAM should be 0 initially"
    );
...
fn test_madsel_write_pixel1() {
    ...
    sys.board.write_ram(0x0003, 0x8D); // STA abs
    sys.board.write_ram(0x0004, 0x00); // addr low
    sys.board.write_ram(0x0005, 0x4D); // addr high (IRQ ack)
    sys.board.write_ram(0x0006, 0xA9); // LDA #imm
    sys.board.write_ram(0x0007, 0x80); // #$80 (data >> 6 = 2 → DATA_LOOKUP[2] = 0xF0, plane1 only)
    sys.board.write_ram(0x0008, 0xA2); // LDX #imm
    sys.board.write_ram(0x0009, 0x00); // #$00
    sys.board.write_ram(0x000A, 0x81); // STA (zp,X)
    sys.board.write_ram(0x000B, 0x10); // zp addr $10
    sys.board.write_ram(0x000C, 0x4C); // JMP abs
    sys.board.write_ram(0x000D, 0x0C);
    sys.board.write_ram(0x000E, 0x00);

    // Pixel address $0801: VRAM addr = $0801 >> 2 = $0200, pixel = 1
    sys.board.write_ram(0x0010, 0x01); // pointer low
    sys.board.write_ram(0x0011, 0x08); // pointer high
...
fn test_get_bit3_addr() {
    ...
    // Result: 0x0200
    assert_eq!(
        MissileCommandBoard::get_bit3_addr(0xE000),
        0x0200,
    ...
    // Result: 0x0400
    assert_eq!(
        MissileCommandBoard::get_bit3_addr(0xE800),
        0x0400,
...
fn test_render_3rd_color_bit() {
    ...
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E); // idx 0: black (R=0,G=0,B=0)
    sys.bus_write(BusMaster::Cpu(0), 0x4B01, 0x0C); // idx 1: blue  (R=0,G=0,B=255)
    ...
    let bit3_base = MissileCommandBoard::get_bit3_addr(0xE000) as usize;
    assert_eq!(bit3_base, 0x0200);

    // Set 3rd color bit for pixel 0: RAM[bit3_base + 0] bit 0 = 1
    sys.board.write_ram(bit3_base, 0x01);

    // Verify the write persisted
    assert_eq!(
        sys.board.read_ram(bit3_base),
        0x01,
        "RAM write at bit3_base should persist"
    );

    // Also verify palette was set
    assert_eq!(sys.board.read_palette(0), 0x0E, "Palette 0 should be 0x0E");
    assert_eq!(sys.board.read_palette(1), 0x0C, "Palette 1 should be 0x0C");
...
fn test_render_no_3rd_color_bit_above_224() {
    ...
    // Set palette entries
    sys.bus_write(BusMaster::Cpu(0), 0x4B00, 0x0E); // idx 0: black
    sys.bus_write(BusMaster::Cpu(0), 0x4B01, 0x0C); // idx 1: blue
    ...
    sys.board.write_ram(0x0401, 0xFF);
