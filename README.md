...
- **ToobinSystem**: Toobin' on its own Atari board (MC68010 + 128×64 scrolling playfield + linked motion objects with SLIP + alpha overlay + 1024-entry palette with a global intensity control + 2048-byte EEPROM + ROT270 portrait display). Video only so far: the JSA-I sound board (M6502 + YM2151 + stereo POKEY) is not modeled yet
