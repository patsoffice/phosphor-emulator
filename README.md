...
Most machines still dispatch through the trait *dynamically*: where a board owns both its CPUs and its bus state in one struct, `cpu.execute_cycle(bus, ..)` cannot borrow-check, and the `bus_split!` macro hands the CPU a `&mut dyn Bus` reborrowed through a raw pointer — so each access is a virtual call the optimiser cannot see through. Boards are moving to the shape that avoids this: CPU state in one field, bus state in another, which borrow-checks natively and monomorphises the whole cycle. The Pac-Man and Galaga families are converted (measured at 5–15% less emulation time per frame, and no `unsafe` left on those paths); see [docs/designs/concrete-bus-dispatch.md](docs/designs/concrete-bus-dispatch.md) for the shape and the rollout.
