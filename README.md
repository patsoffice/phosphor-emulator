...
- ClockTree (each board's crystals in a type, with every sub-clock derived from them rather than restated as a rounded constant. Domains carry both their ratio to the crystal (the auditable hardware statement) and a precomputed ratio to the CPU (what stepping costs, one add-and-compare). Handles boards whose video and CPU clocks sit on different crystals, reports the resulting `cycles_per_scanline` rounding in ppm, and supports runtime retuning of a clock that is not crystal-derived at all (a Votrax VCO, a TMS5220 clock-select bit). Live rates show up in the debugger and the frame overlay)
- ClockDivider (Bresenham fractional clock divider; still used where a rate is not a board clock, e.g. pacing against the host audio rate)
...
- **ToobinSystem**: Toobin' on its own Atari board (MC68010 + 128×64 scrolling playfield + linked motion objects with SLIP + alpha overlay + 1024-entry palette with a global intensity control + 2048-byte EEPROM + ROT270 portrait display + JSA-I sound board on its own crystal)
- **AtariJsa1**: the shared Atari JSA-I sound board (M6502 + YM2151 OPM FM synthesis + optional POKEY + banked program ROM + free-running periodic interrupt + program-controlled mix + the coin switches)
...
- **XeviousSystem**: Xevious on the shared Namco Galaga board (3×Z80 + WSG melodic/effect sound + LLE 54XX explosion channel into its discrete ladders and band-passes; 06XX/51XX I/O + HLE 50XX score/protection; three-layer scrolling video, being a ROM-lookup background, foreground text and 3bpp sprites, with direct DIP reads)
...
- **M68000**: complete instruction set (74 mnemonics), validated against two independently generated suites: [SingleStepTests/680x0](https://github.com/SingleStepTests/680x0) for state, 1,000,058 vectors (every file, every vector incl. address-error aborts), and [SingleStepTests/m68000](https://github.com/SingleStepTests/m68000) for the per-cycle bus trace, exact on instruction length across all 317,500 cases and 98.48% on transfer placement, with 126 of its 127 instructions exact on both. The 68010 variant's timing is datasheet-derived and has no oracle. See [core/src/cpu/m68000/README.md](core/src/cpu/m68000/README.md).
